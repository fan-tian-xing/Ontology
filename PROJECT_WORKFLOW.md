...
4. 原文 Fact 只作为召回补充，优先级低于明确的部件、参数、工序和步骤。
...
对外接口保持为 POST /recommend，请求中传入问题，响应中返回答案、claims 和引用信息。

### 9.2 只修改问答程序

如果只修改 langgraph_app 下的规范化、检索、证据评估、生成或校验逻辑：
...
如果修改 graph_records.jsonl、evidence.jsonl、entity_aliases.json 或 build_corpus.py：

1. 检查新增实体是否属于 15 类节点。
2. 检查关系方向和两端类型是否合法。
3. 检查每条关键关系是否绑定有效 Evidence。
4. 运行 Validator。
5. Validator 通过后运行 Loader。
6. 在 Neo4j 中抽查节点、关系和证据。
7. 运行问答测试。
...
实际参数以各脚本的 --help 和 README.md 为准。重建前要备份正式数据，并确认当前连接的是本项目专用 Neo4j 数据库。

### 9.5 测试重点

每次调整检索、证据或回答逻辑后，至少覆盖：

1. 精确参数或间隙问题。
2. 跨页检查清单。
3. 单一 Procedure 的完整流程。
4. 同时询问多个对象的多目标参数问题。
5. 只有页级 Evidence 的问题。
6. 修配、调整或尺寸异常等高风险问题。
7. Neo4j 或大模型不可用。
8. claim 中页码、章节、数值或单位不一致。

测试代码位于 langgraph_app/tests。

## 10. 当前实现边界

交接和使用时必须了解以下事实：

1. 当前项目只围绕一份指定正式文档建设，不是通用汽轮机知识库。
2. 在线问答运行时不读取 PDF，而是读取已写入 Neo4j 的实体、关系和 Evidence。
3. 用户不能把页码本身当作主要查询方式；系统主要按业务词和实体检索。
4. 正式语料主要由人工整理和构建脚本生成，不是全部由大模型自动抽取。
5. 页级 Evidence 只能用于定位，不能独立证明参数、步骤和工程要求。
6. 完整流程检索必须锁定一个 Procedure，并按 order 展开步骤。
7. 大模型只能使用本次 Neo4j 检索到的证据，不允许用通用知识补写工程结论。
8. 每个 claim 都必须通过页码、章节、数值、单位和来源校验。
9. Loader 会同步并清理本项目中的过期实体、Evidence 和关系，因此应使用项目专用数据库，并谨慎使用 --reset。
10. 涉及修配、调整、尺寸异常和安全风险的结论，即使系统给出依据，也应由具备资质的工程人员复核后执行。

## 11. 项目目录及子目录职责

下面按照 demo 根目录的实际结构说明。业务维护主要关注 docs、evaluation、langgraph_app 和 neo4j；.git 由 Git 自动管理，tmp 是待清理的空临时目录。

~~~text
demo/
├─ docs/                         设计、抽取和质量说明
├─ evaluation/                   问答评估数据与报告
...
│  └─ validator/                 入库前确定性校验
├─ .git/                         Git 版本历史和内部元数据
└─ tmp/pdfs/                     空临时目录，不属于正式项目内容
~~~

### 11.1 docs：项目说明和质量记录

该目录保存知识图谱建设过程中形成的说明文档，不参与在线问答运行，但用于交接、复核和追踪问题。

| 文件 | 用途 |
|---|---|
| document_structure.md | 记录正式资料的章节和文档结构 |
| extraction_pipeline.md | 说明实体、关系和证据的抽取流程 |
| extraction_coverage.md | 记录当前抽取覆盖情况 |
| source_clause_coverage.md | 记录正式条款和原文证据的覆盖情况 |
| source_corrections.md | 记录 OCR 或来源文本的修订和复核说明 |
| ontology_extraction_framework.md | 说明按本体抽取知识的框架 |
| ontology_review.md | 记录本体审核结果和待处理问题 |
| graph_quality_report.md | 记录图谱质量检查结果 |
| runtime_blockers.md | 记录影响系统运行的阻塞问题 |

维护规则：资料、证据或本体发生重要变化时，应同步更新相应说明，避免代码已经变化而交接文档仍停留在旧状态。

### 11.2 evaluation：问答评估资料

该目录用于检查系统是否能从正式资料中找到正确证据并给出可核验回答。

| 文件或文件组 | 用途 |
|---|---|
| expected_answers.json | 典型问题的预期答案或核验要点 |
| blind_cases.json | 盲测问题，避免只针对已知问法调程序 |
| pdf_first_random_questions.json | 从 PDF 内容出发生成的首轮随机问题 |
| pdf_first_random_questions_round2～round9.json | 多轮随机评估问题和迭代记录 |
| evaluation_report.md | 汇总评估结果、问题和改进情况 |

这里的数据不会直接写入 Neo4j。它们的作用是回归测试和质量对比，不应混入正式证据数据。

### 11.3 langgraph_app：在线问答应用
...
.git 保存提交历史、分支、对象和远程仓库信息。其 hooks、info、logs、objects、refs 等子目录均由 Git 自动管理，不属于业务代码，不应手工修改或纳入知识图谱流程。

### 11.6 tmp/pdfs：临时目录

该目录当前为空，不参与构建、入库或在线问答，也不属于正式项目结构。它是明确的临时残留，删除后不会影响项目运行。

## 12. 关键代码位置

| 业务环节 | 代码或数据 |
|---|---|
| Graph Record 通用形状 | neo4j/ontology/schema.json |
| 本体类型和端点说明 | neo4j/ontology/ontology.yaml |
| 运行时确定性白名单 | neo4j/validator/validator.py |
| 正式语料构建 | neo4j/loader/build_corpus.py |
| 可选大模型抽取 | neo4j/loader/extract_graph_records.py |
| 数据校验 | neo4j/validator/validator.py |
| Neo4j 入库 | neo4j/loader/load_graph.py |
| 图记录 | neo4j/data/graph_records.jsonl |
| 证据 | neo4j/data/evidence.jsonl |
| 实体别名 | neo4j/data/entity_aliases.json |
| 问题规范化 | langgraph_app/src/nodes/normalize_question.py |
| 关键词目录 | langgraph_app/src/tools/keyword_catalog.py |
| Neo4j 检索 | langgraph_app/src/nodes/neo4j_retrieval.py、tools/neo4j_tool.py |
| 证据评估 | langgraph_app/src/nodes/assess_evidence.py |
| 答案生成 | langgraph_app/src/nodes/generate_recommendation.py |
| 答案校验 | langgraph_app/src/nodes/validate_response.py |
| LangGraph 工作流 | langgraph_app/src/graph.py、state.py |
| CLI / API | langgraph_app/src/cli.py、api.py |

---

维护时最重要的顺序是：**先保证资料和证据正确，再保证图记录合法，然后入库，最后验证问答结果。**  
系统可以帮助快速定位和组织答案，但不能代替正式资料和工程人员的最终判断。
